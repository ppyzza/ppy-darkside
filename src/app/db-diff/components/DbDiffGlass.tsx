...
export default function DbDiffGlass(props: DbDiffProps) {
  const {
  ...
  return (
          <div key={tableName} style={{ marginBottom: '8px', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', background: '#FFEBEB' }}>
      ...
      if (!tgtCols) {
        ...
        return (
          <div key={tableName} style={{ marginBottom: '8px', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', background: 'rgba(16, 185, 129, 0.1)' }}>
      ...
      const renderDataDiff = () => {
        ...
        return (
          <div style={{ marginTop: '8px', fontSize: '11px', borderTop: '1px solid var(--app-border)', paddingTop: '4px' }}>
            ...
            <div style={{ marginTop: '4px', maxHeight: '100px', overflowY: 'auto', background: '#F9F9F9', padding: '4px', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px' }}>
        ...
        return (
          <div key={tableName} style={{ marginBottom: '4px' }}>
            <div 
              ...
              style={{ padding: '4px 8px', fontSize: '11px', cursor: 'pointer', background: 'rgba(255, 255, 255, 0.1)', border: '1px solid var(--app-panel)', display: 'flex', justifyContent: 'space-between' }}
            >
              <span>{expandedTables[tableName] ? '📂' : '📁'} Table {tableName} <span style={{ color: '#888' }}>(Identical Schema)</span></span>
            </div>
            {expandedTables[tableName] && (
              <div style={{ padding: '8px 16px', fontSize: '11px', color: '#666', background: 'rgba(255, 255, 255, 0.1)' }}>
      ...
      return (
        <div key={tableName} style={{ marginBottom: '8px', border: '1px solid #C98A00' }}>
          ...
          {expandedTables[tableName] && (
            <div style={{ background: 'rgba(255, 255, 255, 0.05)', backdropFilter: 'blur(20px)', padding: '8px' }}>
  ...
  return (
    <div className="app-window" style={{ background: 'rgba(255, 255, 255, 0.05)', backdropFilter: 'blur(20px)', borderRadius: '24px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.1)' }} style={{ height: '100%', maxWidth: '1200px', margin: '0 auto', display: 'flex', flexDirection: 'column' }}>
      ...
      <div className="app-content" style={{ display: 'flex', flexDirection: 'column', flex: 1, padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '16px', background: 'rgba(255, 255, 255, 0.05)', backdropFilter: 'blur(20px)', borderBottom: '1px solid var(--app-border)' }}>
          ...
          <div style={{ display: 'flex', gap: '16px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end' }}>
              <button className="btn btn-primary" style={{ borderRadius: '12px', padding: '8px 24px', background: 'linear-gradient(135deg, var(--app-blue), #6366f1)', border: 'none' }} onClick={handleCompare} disabled={comparing}>
        ...
        {sourceSchema && targetSchema && (
          <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', borderRight: '2px solid var(--app-border)' }}>
              ...
              <div style={{ flex: 1, overflow: 'auto', padding: '8px', background: 'rgba(255, 255, 255, 0.05)', backdropFilter: 'blur(20px)' }}>
                {diffContent}
              </div>
            </div>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
              ...
              <textarea 
                readOnly 
                value={sqlMigration} 
                style={{ flex: 1, width: '100%', border: 'none', resize: 'none', padding: '8px', fontFamily: 'monospace', fontSize: '12px', background: 'rgba(255, 255, 255, 0.1)', whiteSpace: 'pre' }}
              />
            </div>
