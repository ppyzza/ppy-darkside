...
export default function ConfigCompareGlass({
  tenants,
  dbRows,
  csvRows,
  comparison,
    ...
    <div style={{ flex: 1, display: 'flex', gap: '20px', padding: '24px', overflow: 'hidden' }}>
      ...
      <div style={{ 
        ...
        <div>
          ...
          <p style={{ fontSize: '13px', color: 'var(--app-text-muted)' }}>Validate configuration mappings</p>
        </div>

        {/* Tenant Selection */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <label style={{ fontSize: '12px', fontWeight: '600', color: 'var(--app-text-muted)', textTransform: 'uppercase', letterSpacing: '1px' }}>Target Tenant</label>
          <form method="GET" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <select 
              name="tenantId" 
              defaultValue={selectedTenantId || ''} 
              style={{ padding: '12px', borderRadius: '12px', background: 'rgba(255,255,255,0.05)', border: '1px solid var(--app-border)', color: 'var(--app-text)', fontSize: '14px', outline: 'none' }}
            >
            ...
            <button type="submit" style={{ padding: '12px', borderRadius: '12px', background: 'var(--app-blue)', color: 'white', border: 'none', fontWeight: '600', cursor: 'pointer', transition: 'opacity 0.2s' }}>
              Load Configuration
            </button>
          </form>
        </div>
        ...
        {selectedTenantId && (
          <div style={{ 
            ...
            <h3 style={{ fontSize: '14px', fontWeight: '600', marginBottom: '4px' }}>Upload Mapping CSV</h3>
        ...
        {/* Overview Stats */}
        {selectedTenantId && csvRows.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: 'auto' }}>
            <h3 style={{ fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px', color: 'var(--app-text-muted)' }}>Overview</h3>
            <GlassStat label="CSV Records" value={csvRows.length} />
            <GlassStat label="DB Records" value={dbRows.length} />
      ...
      <div style={{ 
        ...
        {!selectedTenantId ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--app-text-muted)', fontSize: '14px' }}>
            Please select a tenant from the sidebar to view configurations.
          </div>
        ) : comparison.length === 0 ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--app-text-muted)', fontSize: '14px' }}>
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: '8px' }}>
            <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: '0 8px' }}>
              <thead style={{ position: 'sticky', top: '-8px', zIndex: 10 }}>
                <tr>
                  <th style={{ padding: '0 16px', fontSize: '12px', fontWeight: '500', color: 'var(--app-text-muted)', textAlign: 'left', border: 'none', background: 'var(--app-bg)', backdropFilter: 'blur(8px)', top: '-8px' }}>Service</th>
